...
- Null-hypothesis significance testing (NHST): data are variable and parameters are fixed
- Bayesian statistics: data are fixed and parameters are variable
...
pivot_wider(samples, names_from = name, values_from = value) %>%
  ...
  scale_x_continuous(limits = c(-100, 500), breaks = seq(-150, 600, 150)) +
  scale_y_continuous(limits = c(-50, 150), breaks = seq(-150, 600, 50)) +
...
- Don't worry if you don't understand what they do, we just need the posterior for the next session.
